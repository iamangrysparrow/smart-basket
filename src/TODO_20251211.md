...
## 3. Улучшение Application Log — ВЫПОЛНЕНО
...
**Решение**:
- [x] `LogEntry` модель с уровнями (Debug, Info, Warning, Error)
- [x] Автоматическое определение уровня по ключевым словам (ERROR, WARN, SUCCESS и т.д.)
- [x] Цветные индикаторы уровней слева от каждой записи
- [x] Фильтрация по уровням — toggle buttons D/I/W/E
- [x] Умный auto-scroll: отключается при scroll up, включается при scroll down
- [x] Кнопка Copy — копирует все отфильтрованные записи в буфер
- [x] Context menu с Copy Selected и Clear All
- [x] Фиксированная высота строк (18px/22px) для плавной виртуализации
- [x] Многострочные сообщения разбиваются на отдельные записи (каждая строка = 1 ListBoxItem)

**Файлы изменены**:
- `Models/LogEntry.cs` — новая модель с LogLevel
- `ViewModels/MainViewModel.cs` — FilteredLogEntries, фильтры, CopyLogCommand, AddLogEntry с split
- `Views/LogWindow.xaml` + `.cs` — полный UI с фильтрами и smart auto-scroll
- `MainWindow.xaml` + `.cs` — встроенная панель с теми же функциями
...
## 8. Потоковое чтение ответов Yandex Agent — ВЫПОЛНЕНО

**Задача**: Добавить streaming для YandexAgentLlmProvider.

**Изменения**:
- [x] Изучена документация REST Assistant API
- [x] Добавлен параметр `stream: true` в запрос
- [x] Реализован парсинг SSE (Server-Sent Events):
  - `data:{json}` + `event:event_type`
  - `response.output_text.delta` — дельты текста
  - `response.completed` — финальное событие
- [x] DTOs для SSE: `StreamEvent`, `StreamResponseInfo`
- [x] Real-time вывод через `IProgress<string>`
- [x] CLI команда `test-agent-provider` для тестирования

**Результат**: Все три LLM провайдера теперь поддерживают streaming:
- `OllamaLlmProvider` ✓
- `YandexGptLlmProvider` ✓
- `YandexAgentLlmProvider` ✓
